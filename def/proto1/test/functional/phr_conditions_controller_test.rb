...
class PhrConditionsControllerTest < ActionController::TestCase
  ...
  def test_actions
    ...
    assert_equal 1, @profile.phr_conditions.reload.size # true = reload
    assert_nil flash[:error]
    ...
    post :create, params: form_data, session: session_data
    assert_redirected_to phr_record_phr_conditions_path
    assert_equal 2, @profile.phr_conditions.reload.size # true = reload

    # Try the research studies link (for basic mode)
    get :studies, params: {:phr_record_id=>@profile.id_shown}, session: session_data
    ...
    post :studies, params: {:phr_record_id=>@profile.id_shown,
      :phr=>{:problem=>'Arm pain', :state_C=>'1-AL', :age_group_C=>1,
      :phr_record_id=>@profile.id_shown}}, session: session_data
    ...
    post :destroy, params: form_data, session: session_data
    assert_redirected_to phr_record_phr_conditions_path
    assert_nil flash[:error]
    assert_equal 1, @profile.phr_conditions.reload.size
  end
