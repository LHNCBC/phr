...
class FormbuilderControllerTest < ActionController::TestCase
  ...
  def test_new_form
    ...
    get :new_form, params: {:form_name=>'FormBuilder'}, session: {:user_id=>users(:phr_admin).id}
    assert_response(:success, @response.body.to_s)

    # confirm that we can't get a new form page if we're not an admin user
    get :new_form, params: {:form_name=>'FormBuilder'}, session: {:user_id=>users(:PHR_Test).id}
  ...
  def test_edit_form
    ...
    # databases
    get :edit_form, params: {:form_name=>'FormBuilder'}, session:
    ...
    get :edit_form, params: {:form_name=>'NotThere'}, session:
                    {:user_id=>users(:phr_admin).id}
    ...
    assert_raise(ActionController::UrlGenerationError) do
      get :edit_form, params: {:form_name=>nil}, session:
                    {:user_id=>users(:phr_admin).id}
    end

    # now try the PHR form
    get :edit_form, params: {:form_name=>'PHR'}, session:
                    {:user_id=>users(:phr_admin).id}
    assert_response(:success, @response.body.to_s)
    #assert_equal(true, @controller.@edit_action)
    #assert_redirected_to('/forms')
  ...
  def test_save_form
    ...
    post :save_form, params: {:fe=>fe_mast.clone}, session:{:user_id=>users(:phr_admin).id}
    assert_response(302, @response.body.to_s)
    ...
    post :save_form, params: {:fe=>fe_mast.clone, :form_name=>'AutoTest'}, session:
                     {:user_id=>users(:phr_admin).id}
    assert_response(302, @response.body.to_s)
...
#    assert_equal(8, fm.label_width, 'label_width')
  ...
  def check_cb_row(display_name)
    ...
    assert_nil( cb.group_header_id, 'check box group_header_id')
    assert_equal('help text for a <em>check box</em>',
                 cb.help_text, 'check box help_text')
    assert_nil( cb.instructions, 'check box instructions')
    ...
    assert_nil( cb.regex_validator_id, 'check box regex_validator_id')
    assert_equal(false, cb.required, 'check box required')
    assert_nil( cb.units_of_measure, 'check box units_of_measure')
  ...
  def check_cne_code_row
    ...
    assert_nil(cd.default_value, 'cne code default_value')
    assert_equal('a cne field code', cd.display_name, 'cne code display_name')
    assert_nil(cd.group_header_id, 'cne code group_header_id')
    assert_nil(cd.help_text, 'cne code help_text')
    assert_nil(cd.instructions, 'cne code instructions')
    ...
    assert_nil(cd.regex_validator_id, 'cne code regex_validator_id')
    assert_equal(false, cd.required, 'cne code NOT required')
    assert_nil(cd.units_of_measure, 'cne code units_of_measure')
