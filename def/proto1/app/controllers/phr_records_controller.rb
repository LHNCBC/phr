...
class PhrRecordsController < BasicModeController
  ...
  before_filter :load_phr_record, :except=>[:index, :archived_index, :new, :create,
    :message_managers]
  before_filter :check_for_autosave, :only=>[:show, :edit]
  ...
  def message_managers
    begin
      ...
      render :json => [reminders_by_profiles].to_json
    rescue Exception => e
      SystemError.record_server_error(e, request, session)
      raise e
    end
