# A Presenter for the account settings page.
class AccountSettingsPresenter < PresenterBase
  # Returns a list of the target field names of the field descriptions used in
  # this page.
  def self.fields_used
    %w{cechngmai_grp ceold_email cenew_email cenew_semail
    cpchngpas_grp cpnew_passwd cpconfm_passwd sqsecquest_grp cp_fixquest
    cp_fixansw cp_selfquest cp_selfansw delete_grp delete_instructions
    delete_account save_changes}
  end
  ...
  def self.current_settings(user)
    form_params = {}
    form_params[:ceold_email_1] = user.email
  ...
  def profile_attr_to_field_id_map
    if !@pattr_to_id 
      map = {}
      map[:email] = :cenew_email_1_1
      map[:email_confirmation] = :cenew_semail_1_1
