...
(function() {
  ...
  this.HTMLtoDOM = function(html, doc) {
    ...
    var initialNode = doc;
    var curParentNode = doc; //moved this up here to make sure the element is actually inserted where I want it inserted (and not appended to the document body like it always is)
    ...
    HTMLParser(html, {
      start: function(tagName, attrs, unary) {
        ...
        // Modified the loop below on 1/14/10 because it was bombing on
        // the toEnglish attribute of our prototype elements.  I think this
        // code did not expect all the attributes added to elements by
        // prototype, much less the ones we add (toEnglish is in application_phr.js)
        // and it just wasn't doing what we expected.
//        for (var attr in attrs)
//          elem.setAttribute(attrs[attr].name, attrs[attr].value);
        for (var a=0, max=attrs.length; a < max; a++) 
          elem.setAttribute(attrs[a].name, attrs[a].value);
      ...
      chars: function(text) {
        if (curParentNode != null && curParentNode != "undefined") {
